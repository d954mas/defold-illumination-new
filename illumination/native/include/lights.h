...
inline void LightsManagerUpdateLights(lua_State* L,LightsManager* lightsManager){
   ...
   lightsManager->lightsVisibleInWorld.SetSize(0);
    ...
    //https://github.com/LanLou123/WebGL-Clustered-Deferred-Forward-Plus-Rendering/blob/master/src/renderers/base.js#L7
    ...
    float zlengthPerCluster = (lightsManager->cameraFar - cameraNear) / lightsManager->zSlice;
    float ystart = -halfY;
    ...
    for (int i = 0; i < lightsManager->lightsVisibleInWorld.Size(); ++i) {
        ...
        float minposz = lightPos.getZ() - cameraNear - lightRadius;
        float maxposz = lightPos.getZ() - cameraNear + lightRadius;
        int zminidx  = floor(minposz / zlengthPerCluster);
        int zmaxidx   = floor(maxposz  / zlengthPerCluster)+1;
        dmLogInfo("lights:%d",l->index);
        dmLogInfo("POS:%f %f %f",lightPos.getX(),lightPos.getY(),lightPos.getZ());
        ...
        for(int j = xminidx + 1; j <= lightsManager->xSlice; ++j) {
            dmVMath::Vector3 norm2 = getNormalComponents(xstart + xlengthPerCluster * j);
            dmVMath::Vector3 norm3 = dmVMath::Vector3(norm2.getX(), 0, norm2.getY());
            if (dmVMath::Dot(lightPos, norm3) < -lightRadius) {
                xmaxidx = fmax(0, j - 1);
        ...
        for(int j = yminidx + 1; j <= lightsManager->ySlice; ++j) {
            dmVMath::Vector3 norm2 = getNormalComponents(ystart + ylengthPerCluster * j);
            dmVMath::Vector3 norm3 = dmVMath::Vector3(0, norm2.getX(), norm2.getY());
            if (dmVMath::Dot(lightPos, norm3) < -lightRadius) {
                ymaxidx = fmax(0, j - 1);
